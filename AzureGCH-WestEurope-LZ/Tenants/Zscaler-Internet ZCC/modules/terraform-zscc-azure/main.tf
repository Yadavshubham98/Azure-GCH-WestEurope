...
resource "azurerm_network_security_group" "cc-mgmt-nsg" {
  count               = var.cc_count
  name                = "GCHPLWEUAZZCC0${count.index + 1}-mgmt-nsg"
...
resource "azurerm_network_security_group" "cc-service-nsg" {
  count               = var.cc_count
  name                = "GCHPLWEUAZZCC0${count.index + 1}-service-nsg"
  location            = var.location
  resource_group_name = var.resource_group
...
resource "azurerm_network_interface" "cc-mgmt-nic" {
  count                     = var.cc_count
  name                      = "GCHPLWEUAZZCC0${count.index + 1}-mgmt-nic"
  location                  = var.location
  resource_group_name       = var.resource_group

  ip_configuration {
    name                          = "GCHPLWEUAZZCC0${count.index + 1}-mgmt-nic-config"
...
resource "azurerm_network_interface" "cc-service-nic" {
  count                           = var.cc_count
  name                            = "GCHPLWEUAZZCC0${count.index + 1}-service-nic"
  location                        = var.location
  resource_group_name             = var.resource_group
  ...
  ip_configuration {
    name                          = "GCHPLWEUAZZCC0${count.index + 1}-service-nic-config"
...
resource "azurerm_network_interface_backend_address_pool_association" "cc-vm-service-nic-lb-association" {
  ...
  network_interface_id    = azurerm_network_interface.cc-service-nic[count.index].id
  ip_configuration_name   = "GCHPLWEUAZZCC0${count.index + 1}-service-nic-config"
...
resource "azurerm_linux_virtual_machine" "cc-vm" {
  count                        = var.cc_count
  name                         = "GCHPLWEUAZZCC0${count.index + 1}"
  location                     = var.location
  resource_group_name          = var.resource_group
  ...
  computer_name                = "GCHPLWEUAZZCC0${count.index + 1}"
...
resource "azurerm_availability_set" "cc-availability-set" {
  ...
  name                         = "GCHPLWEUAZZCC0${count.index + 1}-availability-set"
  location                     = var.location
  resource_group_name          = var.resource_group
