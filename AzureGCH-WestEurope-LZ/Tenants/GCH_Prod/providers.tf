terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "GchWestEuropeLz.tfstate"          //replace with your own file name for the state file ending in .tfstate
  }
}
...
provider "azurerm" {
  use_msi         = true
  tenant_id       = var.tenant_id
  alias           = "genpact-cloud-hosting-sub"
  subscription_id = local.genpact-cloud-hosting-sub
